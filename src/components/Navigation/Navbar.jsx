...
const Navbar = () => {
    ...
    // console.log(document.querySelectorAll('.myline'));
    return (
        <div className='flex fixed top-0 items-start justify-between w-full z-4'>
            ...
            <div onClick={() => {setNavOpen(true)}} onMouseEnter={() => { (navRef.current.style.height = '100%') (document.querySelectorAll('.myline').forEach(elem => {elem.style.backgroundColor = 'black'}))}} onMouseLeave={()=> { (navRef.current.style.height = '0%') (document.querySelectorAll('.myline').forEach(elem => {elem.style.backgroundColor = 'white'}))}} className='bg-black h-12 w-52 relative cursor-pointer'>
                <div ref={navRef} className='bg-[#D3FD50] absolute h-0 w-full transition-all'>
                    
                </div>
                <div  className='bg-black h-full w-full flex flex-col gap-1 items-end justify-center pr-8'>
                    <div className='myline h-0.5 w-10 bg-white z-10'></div>
                    <div className='myline h-0.5 w-7 bg-white z-10'></div>
                </div>
            </div>
        </div>
    )
