...
const Navbar = () => {
    ...
    return (
        <div className='flex fixed top-0 items-start justify-between w-full z-4'>
            ...
            <div onClick={() => {setNavOpen(true)}} onMouseEnter={() => { (navRef.current.style.height = '100%') ()}} onMouseLeave={()=> {navRef.current.style.height = '0%'}} className='bg-black h-12 w-52 relative'>
                ...
                <div  className='bg-black h-full w-full flex flex-col gap-1 items-end justify-center pr-8'>
                    <div className='h-0.5 w-10 bg-white z-10'></div>
                    <div className='h-0.5 w-7 bg-white z-10'></div>
                </div>
            </div>
        </div>
    )
